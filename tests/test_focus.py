...
def test_tmux_focuser(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("ring.focus.shutil.which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr("ring.focus.subprocess.run", fake_run)
...
def test_iterm2_focuser_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("ring.focus.shutil.which", lambda _name: "/usr/bin/osascript")
