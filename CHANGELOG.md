...
- 讓輪詢辨識同一 session 的新等待
- 保留各 agent 的等待並正確配對權限摘要

## 0.17.3 (2026-09-07)

### Fix

- 讓等待狀態不被背景 subagent 與並行 hook 誤清
- 防止同目錄的新 Claude 復活舊等待狀態
...
- 讓等你的 session 不會放著 60 秒就自己變成跑完停著

## 0.17.0 (2026-08-16)

### Feat

- 讓 doctor 抓出 Claude Code 自己也在發的通知

## 0.16.0 (2026-08-13)

### Feat

- 支援在 kitty 的 TUI 內回覆權限請求
- 支援跳轉回 kitty 終端

### Fix

- 修好選項一折行就整份權限對話框讀不到
- 讓 tmux session 跳轉也把外層終端視窗帶到前景

## 0.15.1 (2026-08-04)

### Refactor

- 把 tmux 掃描與路徑正規化從 registry 拆成獨立模組
